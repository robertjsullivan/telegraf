...
func newWavefrontClient(cfg *configuration) (Sender, error) {
	metricsReporter := notinternal.NewReporter(fmt.Sprintf("%s:%d", cfg.Server, cfg.MetricsPort), cfg.Token)
	tracesReporter := notinternal.NewReporter(fmt.Sprintf("%s:%d", cfg.Server, cfg.TracesPort), cfg.Token)
	...
	sender.pointHandler = newLineHandler(metricsReporter, cfg, notinternal.MetricFormat, "points", sender.internalRegistry)
	sender.histoHandler = newLineHandler(metricsReporter, cfg, notinternal.HistogramFormat, "histograms", sender.internalRegistry)
	sender.spanHandler = newLineHandler(tracesReporter, cfg, notinternal.TraceFormat, "spans", sender.internalRegistry)
	sender.spanLogHandler = newLineHandler(tracesReporter, cfg, notinternal.SpanLogsFormat, "span_logs", sender.internalRegistry)
	sender.eventHandler = newLineHandler(metricsReporter, cfg, notinternal.EventFormat, "events", sender.internalRegistry)
...
func (sender *wavefrontSender) initializeInternalMetrics(cfg *configuration) {
	...
	var setters []notinternal.RegistryOption
	setters = append(setters, notinternal.SetPrefix("~sdk.go.core.sender.direct"))
	setters = append(setters, notinternal.SetTag("pid", strconv.Itoa(os.Getpid())))

	for key, value := range cfg.SDKMetricsTags {
		setters = append(setters, notinternal.SetTag(key, value))
	}
