...
func MetricLine(name string, value float64, ts int64, source string, tags map[string]string, defaultSource string) (string, error) {
	...
	sb := notinternal.GetBuffer()
	defer notinternal.PutBuffer(sb)
...
func sanitizeInternal(str string) string {
	...
	skipHead := 0
	if strings.HasPrefix(str, notinternal.DeltaPrefix) {
		sb.WriteString(notinternal.DeltaPrefix)
		skipHead = 3
	}
	if strings.HasPrefix(str, notinternal.AltDeltaPrefix) {
		sb.WriteString(notinternal.AltDeltaPrefix)
		skipHead = 2
	}
	// Second character can be ~ tilda character if first character
	// is \u2206 (∆ - INCREMENT) or \u0394 (Δ - GREEK CAPITAL LETTER)
	if (strings.HasPrefix(str, notinternal.DeltaPrefix) || strings.HasPrefix(str, notinternal.AltDeltaPrefix)) &&
		str[skipHead] == 126 {
