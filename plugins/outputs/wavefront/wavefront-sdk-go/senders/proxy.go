...
type proxySender struct {
	handlers         []notinternal.ConnectionHandler
	...
	pointsDiscarded *notinternal.DeltaCounter
...
func NewProxySender(cfg *ProxyConfiguration) (Sender, error) {
	sender := &proxySender{
		defaultSource: notinternal.GetHostname("wavefront_proxy_sender"),
		handlers:      make([]notinternal.ConnectionHandler, handlersCount),
	}

	var setters []notinternal.RegistryOption
	setters = append(setters, notinternal.SetPrefix("~sdk.go.core.sender.proxy"))
...
func makeConnHandler(host string, port, flushIntervalSeconds int, prefix string, internalRegistry *notinternal.MetricRegistry) notinternal.ConnectionHandler {
	addr := host + ":" + strconv.FormatInt(int64(port), 10)
	flushInterval := time.Second * time.Duration(flushIntervalSeconds)
	return notinternal.NewProxyConnectionHandler(addr, flushInterval, prefix, internalRegistry)
}
